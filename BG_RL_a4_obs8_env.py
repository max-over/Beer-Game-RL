...
class BeerGameEnv(gymnasium.Env):
    ...
    def step(self, action):
        ...
        reward = -(self.ret.periodcosts + self.distr.periodcosts + self.whole.periodcosts + self.plant.periodcosts) / 1000

        if self.current_period >= self.periods:
            done = True
            reward2 = -(self.ret.costs + self.distr.costs + self.whole.costs + self.plant.costs)/1000
            print(f"total costs: {reward2*1000}")
