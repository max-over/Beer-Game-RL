...
class BeerGameEnv(gymnasium.Env):
    ...
    def __init__(self):
        ...
        self.ret.backlograte = 1000 #10 #1000
    ...
    def step(self, action):
        ...
        alpha = 0.02
        bwe = (max(0,action[0] - self.ret.current_demand) + max(0, action[1] - action[0]) + max(0, action[2] - action[1]) + max(0,action[3] - action[2]))/4
        costs = (self.ret.periodcosts + self.distr.periodcosts + self.whole.periodcosts + self.plant.periodcosts) / 10000
        reward = -((1-alpha)*costs + alpha*bwe)
